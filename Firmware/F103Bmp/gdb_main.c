...
typedef enum gdb_signal {
    GDB_SIGINT = 2, GDB_SIGTRAP = 5, GDB_SIGSEGV = 11, GDB_SIGLOST = 29,
} gdb_signal_e;
...
static void gdb_target_destroy_callback(target_controller_s *tc, target_s *t) {
    ...
    if(last_target == t) last_target = NULL;
}

static void gdb_target_printf(target_controller_s *tc, const char *fmt,
        va_list ap) {
    (void) tc;
    gdb_voutf(fmt, ap);
}

target_controller_s gdb_controller = { .destroy_callback =
        gdb_target_destroy_callback, .printf = gdb_target_printf,

.open = hostio_open, .close = hostio_close, .read = hostio_read, .write =
        hostio_write, .lseek = hostio_lseek, .rename = hostio_rename, .unlink =
        hostio_unlink, .stat = hostio_stat, .fstat = hostio_fstat,
        .gettimeofday = hostio_gettimeofday, .isatty = hostio_isatty, .system =
                hostio_system, };

/* execute gdb remote command stored in 'pbuf'. returns immediately, no busy waiting. */

int gdb_main_loop(target_controller_s *tc, char *pbuf, size_t pbuf_size,
        size_t size, bool in_syscall) {
    ...
    switch(pbuf[0]) {
        ...
        case 'm': { /* 'm addr,len': Read len bytes from addr */
            ...
            gdb_putpacketz("E01");
            else gdb_putpacket(hexify(pbuf, mem, len), len * 2U);
            break;
        }
        ...
        case 'p': { /* Read single register */
            ...
            if(cur_target->reg_read) {
                ...
                if(s > 0) gdb_putpacket(hexify(pbuf, val, s), s * 2U);
                else
                gdb_putpacketz("EFF");
            }
            else {
        ...
        case 'F': /* Semihosting call finished */
            if(in_syscall) return hostio_reply(tc, pbuf, size);
            else {
        ...
        case 'R': /* Restart the target program */
            if(cur_target) target_reset(cur_target);
            else if(last_target) {
...
static bool exec_command(char *packet, const size_t length,
        const cmd_executer_s *exec) {
...
static void exec_q_rcmd(const char *packet, const size_t length) {
    ...
    gdb_putpacketz("OK");
        ...
        gdb_putpacket(hexify(pbuf, response, response_length),
                2 * response_length);
    }
}
...
static void exec_q_crc(const char *packet, const size_t length) {
    ...
    if(sscanf(packet, "%" PRIx32 ",%" PRIx32, &addr, &addr_length) == 2) {
        ...
        gdb_putpacketz("E03");
        else gdb_putpacket_f("C%" PRIx32, crc);
    }
}
...
static const cmd_executer_s q_commands[] = { { "qRcmd,", exec_q_rcmd }, {
        "qSupported", exec_q_supported }, { "qXfer:memory-map:read::",
        exec_q_memory_map }, { "qXfer:features:read:target.xml:",
        exec_q_feature_read }, { "qCRC:", exec_q_crc }, { "qC", exec_q_c }, {
        "qfThreadInfo", exec_q_thread_info }, { "qsThreadInfo",
        exec_q_thread_info }, { "QStartNoAckMode", exec_q_noackmode }, {
        "qAttached", exec_q_attached }, { NULL, NULL }, };
...
static void handle_q_packet(char *packet, const size_t length) {
    if(exec_command(packet, length, q_commands)) return;
    DEBUG_GDB("*** Unsupported packet: %s\n", packet);
    gdb_putpacket("", 0);
}
...
static void handle_v_packet(char *packet, const size_t plen) {
    ...
    if(sscanf(packet, "vAttach;%08" PRIx32, &addr) == 1) {
        ...
        cmdline[0] = '\0';
        while(*tok != '\0') {
            if(strlen(cmdline) + 3U >= sizeof(cmdline)) break;
            if(*tok == ';') {
                *pcmdline++ = ' ';
                pcmdline[0] = '\0';
                tok++;
    ...
    else if(sscanf(packet, "vFlashErase:%08" PRIx32 ",%08" PRIx32, &addr, &len)
            == 2) {
        /* Erase Flash Memory */
        ...
        if(cur_target
                && target_flash_write(cur_target, addr, (void*) packet + bin,
                        count))
        gdb_putpacketz("OK");
        else {
            target_flash_complete(cur_target);
...
static void handle_z_packet(char *packet, const size_t plen) {
    ...
    uint32_t len;
    uint32_t addr;
    sscanf(packet, "%*[zZ]%" PRIu32 ",%08" PRIx32 ",%" PRIu32, &type, &addr,
            &len);

    int ret = 0;
    if(packet[0] == 'Z') ret = target_breakwatch_set(cur_target, type, addr,
            len);
    else ret = target_breakwatch_clear(cur_target, type, addr, len);
...
void gdb_halt_target(void) {
    if(cur_target) target_halt_request(cur_target);
    else
